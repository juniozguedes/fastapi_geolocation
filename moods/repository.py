...
def create_mood(db: Session, mood_schema: MoodResponse):
    query = Mood(
        mood=mood_schema.mood,
        latitude=mood_schema.latitude,
        longitude=mood_schema.longitude,
...
def mood_frequency(db: Session, user_id: int):
    ...
    query = db.query(
        subquery.c.user_id,
        subquery.c.mood,
        subquery.c.count.label("frequency:")
    ).all()
...
def get_specific_moods(db: Session, mood_type: str, user_id: int):
    query = db.query(Mood).filter(
        Mood.user_id == user_id, Mood.mood == mood_type).all()
    return query
